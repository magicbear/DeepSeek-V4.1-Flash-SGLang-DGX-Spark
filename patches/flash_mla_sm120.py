...
def flash_mla_with_kvcache_sm120(**kwargs):
    ...
    if _sm120_default_backend == "flashinfer":
        if q.shape[0] > SM120_DECODE_MAX_TOKENS and os.environ.get(
            "DSV41_PREFILL_TRITON"
        ):
            # FlashInfer 0.7's DSV4 prefill specialization is both slower on
            # GB10 and unsupported for some page/top-k combinations.  Keep
            # its fast split-K decode kernel, but use the established Triton
            # implementation for large query batches.
...
def _flash_mla_flashinfer(
    ...
    q_decode = q.squeeze(1) if q.ndim == 4 else q
    if B <= _FI_DECODE_MAX_TOKENS:
        # The generic 0.7 dispatcher is allowed to cross over to a prefill
        # kernel.  DSV4's page-64/topk-128 combination has no such prefill
        # variant, so graph capture can fail even for a decode batch.  Invoke
        # the same standalone split-K DSV4 kernel used by vLLM directly.
        sparse_mla_sm120_decode_dsv4(
            q_decode,
            kv_64,
            idx,
