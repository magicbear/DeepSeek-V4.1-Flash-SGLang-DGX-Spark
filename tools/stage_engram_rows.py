...
if not bad:
    record = {'source_index_sha256': hashlib.sha256(open(f"{src}/model.safetensors.index.json",'rb').read()).hexdigest(),
              'row_checks': checked, 'mismatches': bad, 'layers': layers,
              'reference_commit': 'ca662ac35193c69ace9cee37f13a94abf2eff0fc'}
    with open(f"{dst}/verified.json.tmp",'w') as f:
        json.dump(record,f,indent=2)
    os.replace(f"{dst}/verified.json.tmp",f"{dst}/verified.json")
