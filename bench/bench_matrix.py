...
"""
import argparse, json, random, statistics as st, threading, time, urllib.request, re
from concurrent.futures import ThreadPoolExecutor
from pathlib import Path
import uuid
from contextlib import contextmanager

# Adapted from tonyd2wild's MIT-licensed benchmark at ca662ac35193c69ace9cee37f13a94abf2eff0fc.
# Preserve raw SSE and write incremental checkpoints so a failed cell loses no earlier data.
RAW_DIR = None

@contextmanager
def preserve_failed_request(body, events):
    try:
        yield
    except BaseException as exc:
        if RAW_DIR:
            (RAW_DIR/('error-'+uuid.uuid4().hex+'.json')).write_text(json.dumps(
                {'request':body,'events':events,'error':repr(exc)},ensure_ascii=False,indent=2))
        raise
...
def stream_chat(base, model, prompt, max_tokens, timeout=300):
    body = {"model": model, "messages": [{"role": "user", "content": prompt}], "max_tokens": max_tokens,
            "temperature": 0, "stream": True, "stream_options": {"include_usage": True},
            "chat_template_kwargs": {"thinking": False}}
    req = urllib.request.Request(base + "/chat/completions", data=json.dumps(body).encode(),
                                 headers={"Content-Type": "application/json"})
    t0 = time.perf_counter(); ttft = None; usage = None; chars = 0
    events = []; text = ''; finish_elapsed = None
    with preserve_failed_request(body, events), urllib.request.urlopen(req, timeout=timeout) as r:
        for raw in r:
            ...
            events.append({'elapsed_s':elapsed,'data':ev})
            if ev.get("usage"):
                usage = ev["usage"]
            for ch in ev.get("choices") or []:
                ...
                if ch.get('finish_reason') is not None:
                    ...
                    if ch['finish_reason'] in ('abort','error'):
                        raise RuntimeError(f"Generation failed: {ch}")
        if not usage or not usage.get('completion_tokens'):
            raise RuntimeError('Missing valid server token usage')
    total = time.perf_counter() - t0
    ct = (usage or {}).get("completion_tokens", 0); pt = (usage or {}).get("prompt_tokens", 0)
    end = finish_elapsed if finish_elapsed is not None else total
    dec = (ct - 1) / (end - ttft) if (ttft is not None and ct > 1 and end > ttft) else None
    result = {"ttft_s": ttft, "total_s": total, "completion_tokens": ct, "prompt_tokens": pt,
              "decode_tok_s": dec, "chars": chars, "finish_elapsed_s":finish_elapsed, "text":text}
    if RAW_DIR:
        rawpath = RAW_DIR/(uuid.uuid4().hex+'.json')
        rawpath.write_text(json.dumps({'request':body,'events':events,'result':result},ensure_ascii=False,indent=2))
        result['raw_file']=str(rawpath)
    return result
...
def run_batch(args, c, cat, prompt, max_tokens, tag):
    ...
    for r in res:
        t=r['text']
        if cat=='ceiling_count': r['output_check']=([int(x) for x in re.findall(r'\d+',t)]==list(range(1,81)))
        elif cat=='coding': r['output_check']='def merge_intervals(' in t
        elif cat=='prose': r['output_check']=sum(x in t.lower() for x in ['heat','refrigerant','compressor','evapor','warmth'])>=2
        else: r['output_check']=None
...
def main():
    ...
    ap.add_argument("--levels", default="1,2,3,4,5,6,7,8"); ap.add_argument("--prefill", default="2000,4000")
    ap.add_argument('--categories', default='')
    ap.add_argument('--warm-prefill', action='store_true')
    ap.add_argument("--notes", default=""); ap.add_argument("--dump-prompts")
    args = ap.parse_args()
    Path(args.out).mkdir(parents=True,exist_ok=True)
    RAW_DIR=Path(args.out)/('raw-'+args.label); RAW_DIR.mkdir(exist_ok=True)
    if args.categories:
        wanted=set(args.categories.split(',')); CATEGORIES=[x for x in CATEGORIES if x[0] in wanted]
    ...
    levels = [int(x) for x in args.levels.split(",") if x]; pre = [int(x) for x in args.prefill.split(",") if x]
    res = {"label": args.label, "prompt_set": PROMPT_SET_VERSION, "model": args.model, "notes": args.notes,
           "started": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
           "levels": levels, "categories": [x[0] for x in CATEGORIES] + [CEILING[0]],
           "prefill_shape_warmed": args.warm_prefill, "batches": [], "prefill": []}
    checkpoint_path=Path(args.out)/f'bench-{args.label}.json'
    def checkpoint():
        checkpoint_path.write_text(json.dumps(res,ensure_ascii=False,indent=2))
    checkpoint()
    ...
    for c in levels:
        for n, p, m in CATEGORIES + [CEILING]:
            ...
            if any(r.get('output_check') is False for r in b['requests']):
                res['status']='stopped_output_check_failure';res['valid_for_performance_comparison']=False;checkpoint()
                raise SystemExit('Output check failed; preserving this cell and stopping the matrix')
    for tgt in pre:
        if args.warm_prefill:
            warm=stream_chat(args.base,args.model,f'[shape warm {tgt}] '+filler(int(tgt*.55),tgt+12345)+'\nReply OK.',1,timeout=600)
            res.setdefault('prefill_shape_warmups',[]).append(warm)
            checkpoint()
        r = stream_chat(args.base, args.model, f"[bench {PROMPT_SET_VERSION} prefill {tgt}] " + filler(int(tgt * 0.55), tgt)
                        + "\n\nReply with the single word OK.", 1, timeout=600)
        row = {"target": tgt, "prompt_tokens": r["prompt_tokens"], "ttft_s": round(r["ttft_s"], 3),
               "prefill_tok_s": round(r["prompt_tokens"] / r["ttft_s"], 1) if r["ttft_s"] else None,
               "raw_file":r.get('raw_file'), "metric_note":"Effective prompt tokens / client TTFT; includes scheduler, network and first-token overhead."}
        res["prefill"].append(row); print(f"  prefill {tgt:>6}: {row}", flush=True)
        checkpoint()
    res["finished"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    res['status']='completed';res['valid_for_performance_comparison']=True
    head = []
    for c in levels:
    ...
    md = [f"## {args.label} ({res['started']})", "", args.notes, "",
          ...
          f"### Throughput by concurrency ({len(CATEGORIES)} categories; the counting ceiling is excluded)", "",
